 ...
 * 保存开奖记录
 */
const saveLotteryRecord = async (redis, rootMessageId, winnerId, participantCount, chatId, senderId, lotteryMessageId) => {
    const key = getLotteryKey(rootMessageId);
    const record = {
        winnerId,
        participantCount,
...
const recordLottery = async (redis, logger, winnerData) => {
    try {
        ...
        logger.info(`记录详情: 中奖用户=${winnerData.winnerId}, 参与人数=${winnerData.participants.length}, 群聊=${winnerData.chatId}, 触发人=${winnerData.senderId}`);
